...
Monitor tools are always available and expose the public `/v1/api/monitors` API for creating, configuring, and reading monitor output.
...
| `create_monitor` | Create a DRAFT or ACTIVE monitor with typed query, output schema, schedule, and delivery configuration. Defaults to DRAFT. |
| `update_monitor` | Partially update monitor configuration while preserving omitted fields. |
