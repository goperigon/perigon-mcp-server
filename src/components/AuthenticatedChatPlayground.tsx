...
import { useAuthentication, createAppStateSnapshot } from "../contexts/AuthenticationContext";

export default function AuthenticatedChatPlayground() {
  const auth = useAuthentication();
  const [isPreservingState, setIsPreservingState] = useState(false);
  
  const { messages, input, handleInputChange, handleSubmit, status, error, setMessages, setInput } =
    useChat({
      api: "/v1/api/chat",
      fetch: auth.createChatFetchFunction(),
      initialMessages: [
        {
          id: "1",
          role: "assistant",
...
What would you like to explore?`,
      ...
      onError: (error) => {
        if (error.message.includes('Authentication required')) {
          // Preserve chat state before re-authentication
          const chatState = createAppStateSnapshot(
            messages,
            input,
            null, // selectedTool - not applicable for chat
            {}, // toolParams - not applicable for chat
            null, // executionResult - not applicable for chat
            'chat' // activeTab
          );
          auth.preserveCurrentState(chatState);
          setIsPreservingState(true);
        }
      }
    });
  ...
  // Restore chat state after successful authentication
  useEffect(() => {
    if (auth.isAuthenticated && isPreservingState) {
      const restoredState = auth.restoreState();
      if (restoredState && restoredState.chatMessages) {
        console.log('Restoring chat conversation state');
        setMessages(restoredState.chatMessages);
        setInput(restoredState.chatInput);
        setIsPreservingState(false);
        auth.clearPreservedState();
      }
    }
  }, [auth.isAuthenticated, isPreservingState]);
  ...
  const handleKeyPress = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      if (input.trim() && status === "ready" && auth.isAuthenticated) {
      ...
      <div className="fixed inset-0 top-12 flex flex-col bg-background">
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-muted-foreground text-sm font-mono">
            ...
            <p>AUTHENTICATING...</p>
            <div className="text-xs mt-2">Please complete verification to start chatting</div>
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="flex space-x-3">
            <Textarea
              ...
              disabled={status !== "ready" || !auth.isAuthenticated}
            />
            <Button
              type="submit"
              disabled={status !== "ready" || !input.trim() || !auth.isAuthenticated}
...
// Tool call visualization component (same as original)
