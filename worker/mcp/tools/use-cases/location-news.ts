...
export const locationNewsArgs = z.object({
  location: z
    ...
    .min(1)
    .describe(
      "Location to search for news about (city, state, or country name)",
    ),
  days: z
    .number()
    .int()
...
export function getLocationNews(
  perigon: Perigon,
): ToolCallback<typeof locationNewsArgs> {
  return async ({
    location,
    days,
    limit,
    ...
    try {
      ...
      if (locationType === "auto") {
        ...
        if (location.length === 2 && usStates.includes(locationLower)) {
          ...
          location.length === 2 &&
          commonCountries.includes(locationLower)
        ) {
      ...
      if (newsResult.numResults === 0) {
        return toolResult(
          `No recent news found for "${location}" in the last ${days} days.`,
        );
      }
...
Journalist Ids: ${journalistIds}
      ...
      const detectedType = searchParams.city
        ? "city"
        : searchParams.state
          ? "state"
