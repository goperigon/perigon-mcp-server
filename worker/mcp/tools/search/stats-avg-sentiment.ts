...
export function getAvgSentiment(
  perigon: Perigon,
): ToolCallback<typeof avgSentimentArgs> {
  return async (
    args: z.infer<typeof avgSentimentArgs>,
  ): Promise<CallToolResult> => {
    try {
      const result: StatResult<AvgSentimentStatDto> =
        await perigon.getAvgSentiment({
          q: args.q,
          from: args.from,
          to: args.to,
          source: args.source,
