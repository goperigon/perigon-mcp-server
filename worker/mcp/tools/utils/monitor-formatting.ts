...
function formatJsonWithinLimit(
  ...
  return {
    json: JSON.stringify(
      {
        _truncated: true,
        _originalCharacterCount: json.length,
        _message:
          "Configuration exceeds the display limit and was omitted to prevent unsafe updates."
      },
      null,
      2
    ),
    notice:
      "Configuration was omitted in full. Do not use this placeholder as an update_monitor payload; update only explicitly requested fields."
  };
}
...
export function formatMonitorDetail(monitor: MonitorDto): string {
  ...
  const { json: configurationJson, notice: configurationNotice } =
    formatJsonWithinLimit(configuration, MAX_MONITOR_CONFIG_CHARS);
